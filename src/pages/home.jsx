...
function Home() {
    ...
    const handleDetails = (image) => {
        window.scrollTo(0, 0);
        navigate(`/details/${image.id}`, { state: { image, images } });
    };
    ...
    return (
        <div className={`p-4 min-h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-900"}`}>
            <div className="flex justify-between items-center gap-5">
                ...
                <form onSubmit={handleSearch} className={`mb-6 flex items-center shadow-md rounded-full overflow-hidden w-full max-w-lg mx-auto border ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-300"}`}>
                    ...
                    <input
                        ...
                        placeholder="Qidiruv..."
                        className={`w-full py-3 ${darkMode ? "bg-gray-800 text-white placeholder-gray-400" : "bg-white text-gray-700"} focus:outline-none`}
                    />
                    <button type="submit" className={`px-4 py-3 rounded-r-lg ${darkMode ? "bg-gray-600 hover:bg-gray-700 text-white" : "bg-gray-500 hover:bg-gray-700 text-white"}`}>
                        Qidirish
                    </button>
                </form>
            </div>
